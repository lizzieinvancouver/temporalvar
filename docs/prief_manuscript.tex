...
Yet mechanistically characterizing this tradeoff is a challenge. This is because phenology is not a trait, but rather the realized product of an interaction between the environment (cues) and species unique physiology and genetic and developmental attributes. Environmental cues are changing rapidly, in fact phenological shifts are one of the most pronounced indicators of climate change to date \citep{}. However species' phenological responses to climate change differ significantly. We suggest that the degree to which a species responds to environmental variation known as \textbf{phenological sensitivity} ($\Delta$ phenology/ $\Delta$ environmental cue) is the primary mediator of the tradeoff between phenology and competition, and understanding this trait is key to predicting community assembly in an era of global change. 
...
% Alternative [replace next 3-4 sentence -- until the sentence that starts `Models have done this through an...' -- with just this one sentence]: Building off experimental \citep{godoy2014} and observational \citep{wolkovich2013} evidence that earlier species appear to perform better models have generally built in trade-off where species with a timing advantage also experience greater competition. 
...
Predicting phenological responses to a changing environment requires approaches that allow species phenology to vary with the environment based on the underlying physiology. For most species, this means understanding how the environment is shifting and how those shifts filter through species-specific cue systems (Chmura et al 2019 and Bonamour et al. 2019). For the many spring plant events, such as leafout and seed germination, that cue to cool winter temperatures and warm spring temperatures integrating both of these components is likely critical for useful assembly forecasts because of its effects on realized phenology. Integrating both cues into a simple assembly model, we found realized phenology can vary xx-yy.
...
Then maybe a paragraph that says, 'building on this interactive knowledge of what determines realized phenology can help us better understand how communities may shift with climate change.' [Then I would broadly talk about what could happen in more basic terms: some species benefit from shifting cues by earlier and greater access to resources, such as suggested for invasions, but the full result depends on what stabilizes the community to begin with. If the community is assembled via some aspect of phenology, then a shifting environment could reshape that.] Then say briefly what we see [basically try to skip all methods and/or tuck in caption] stressing that you have different communities depending on 6 or 12 weeks. 

But what doesn't change? The fundamental tradeoff between realize phenological differences and competitive differences does not change in the new environment (Figure \ref{Fig:coexistence}a). 
...
% emw13Jun -- I think weaving in ecological implications as we sneak in our results throughout this section could work (I think) but we should discuss Monday if there is time about how to do it to see if we really think it would work. 
