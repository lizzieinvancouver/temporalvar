...
{\bf Next meeting:} One more meeting on parameter space. Then we should have a meeting that is longer and `tactical' --- figuring out how to edit the code. \\ % Megan says Lizzie does not need to come (but can come!)

\emph{8 December 2021} We mainly edited the R code (\verb|getEnvt.R| -- semi-done, may someday add copula; and \verb|getSpecies.R| made some progress but not done -- we stopped around line 23 where we need to think about how to add within-season stuff, see `\#\#NEXT'), talking about the distance between timepoints and DR curves along the way. We did not review the other code and we did not adjust the timestep issue in the ODE. \\
