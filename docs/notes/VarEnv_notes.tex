...
\date{Last updated:  16 August 2015}
\maketitle
...
\subsection{Write-up current state of progress for mechanisms of coexistence} 
Megan will write up where she arrived at as of 8 August 2015 working off Chesson 1994 (TPB) and Chesson 2003 (TPB also I think). See file \verb|VarEnv_CoexistEqns.tex|.

\subsection{Get $R^*$ model running by 7 September 2015}
So, Megan needs to (1) confirm issue we left last were working on with model and (2) help solve it (note, we think the problem was how to get ODE solver to end a season for each species when it reaches its $R^*$ value). 

\subsection{Get parameter simulations running by 7 Septemner 2015}
So, Lizzie needs to get the code to run through sweeps of parameters running, including figuring out how to get it up on Research Computing.
...
\emph{Old note, does this still apply?} First, to make life easy I (Lizzie) will swap the hats when coding. So what we call \(\hat{\tau_{i}}\) I will call \(\tau_{i}\) and vice versa.

\subsection{Writing tasks for Lizzie}
(1) Write out clear questions to address in each paper and (2) add references for Chesson citations above and below. Also, maybe (3) remove equations from Feb-Jan 2015 below. And add in any references to other files that Megan works up. \emph{And then read all these notes over for clarity etc..}
...
\section{Notes from 8 August 2015}
...
\section{Notes on coexistence mechanisms}
...
\subsection{Three-part approach to project}

Possible titles: `Phenological tracking: It's more complicated than you think' (we hope) or `Phenological tracking: Is it naive?'
\begin{enumerate}
...
\end{enumerate}
\end{enumerate}
\item Are there environmental conditions under which tracking won't work as a strategy?
\begin{enumerate}
...
We broke down this trade-off to: Wider (probably shorter) germination curves trade off with shorter seedbank life such that ...
\begin{smitemize}
...
\begin{figure}[h!]
\centering
\noindent \includegraphics[width=0.9\textwidth]{/Users/Lizzie/Documents/git/temporalvar/figures/figure_tradeoff.png}
...
As of August 2015 we decided {\bf against doing tracking with join distributions}, but here are the old notes on it:
\begin{quote}
