...
We have a few goals:
\begin{smitemize}
\item Understand: why so many more 2-spp. coexist after stationary in $\tau_i x R*$ runs versus $\alpha x R*$? And how come the bottleneck in the $\alpha x R*$ and not the $\tau_i x R*$? (Some notes on this in phencc.tex file currently.)
\item Understand:  why so few 2-spp. coexist after non-stationary in $\tau_i x R*$ runs versus $\alpha x R*$
\item Lizzie should finish the $R_0$ run estimates.

\item Do a few megadrought runs, maybe enough to get a sense of how big the megadrought versus non-megadrought periods would need to be for our hypothesis to work.
\end{smitemize}
