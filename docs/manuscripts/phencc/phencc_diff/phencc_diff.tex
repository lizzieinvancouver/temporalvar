...
%DIF DEL phencc_old.tex   Fri Apr 30 13:36:03 2021
%DIF ADD phencc.tex       Fri Apr 30 13:34:15 2021
...
}\DIFaddend . If researchers measure some related attribute \DIFdelbegin \DIFdel{(e.g., mean spring temperature in place of thermal sums) }\DIFdelend or only some of the organism's cues, then the organism will appear to track poorly (i.e., a noisier statistical relationship\DIFdelbegin \DIFdel{from poor measurement quality).  Most empirical studies, however, }\DIFdelend \DIFaddbegin \DIFadd{).  Aside from a few model systems \mbox{%DIFAUXCMD
...
Other models canalize species' responses to the environment into production and investment. Most models of inter-annual competition \citep[most explicit examples of `modern coexistence theory,' e.g.,][]{Chesson:2004eo,Angert:2009} fall into this camp. Species produce (via investment in offspring, tissue etc.) differentially depending on the environment each year and outcomes are mediated through density. \DIFaddbegin \R{r2thisrelates} \DIFaddend While these models superficially may seem disconnected from timing, they \DIFdelbegin \DIFdel{highlight how phenology }\DIFdelend \DIFaddbegin \DIFadd{are built on the idea that how well species are `matched' to the environment varies across both species and years, and determines fitness. A few explicitly define this `match' based on phenology \mbox{%DIFAUXCMD
...
. This provides hope for a more general framework where cue systems can more quickly be identified based. Such a framework would also allow forecasts that include the shifting genetics of phenology as species shift their ranges with climate change \mbox{%DIFAUXCMD
...
\subsection{\DIFadd{Reject assumptions of stationarity and embrace non-stationarity}} 
...
. Such stories of the past are now happening today, and has caused ecologists to question their simplifying assumption of stationarity \mbox{%DIFAUXCMD
\citep{christy2021}}%DIFAUXCMD
. We argue that better predictions of climate change impacts---and fundamental insights---will come from embracing the complexity of non-stationary environments, and transitions into them, across ecological levels and fields.
}
...
\hline \hline
\emph{\DIFaddFL{community assembly --}}  \DIFaddFL{the suite of processes that determine which species are in a community---this includes processes that determine the regional species pool and processes that determine which species persist, including stabilizing and equalizing mechanisms  }\\ \hline
\emph{\DIFaddFL{cue reliability --}} \DIFaddFL{the match between ideal timing and actual timing (see Figs. \ref{fig:defineETorig}-\ref{fig:defineET})}\\  \hline
\emph{\DIFaddFL{environmental tracking --}}  \DIFaddFL{the change in timing of a biological event due to an organism's cue system given change in the environment  (Fig. \ref{fig:defineET}, note the shift in timing between sites); for example, considering a tree where budburst is determined by a combination of chilling, forcing and photoperiod cues---its environmental tracking would the number of days shift in the timing of budburst in response to a change in environmental conditions, such as warmer winters and springs }\\  \hline %DIF >  By definition it includes any factors that cue an event; thus if an an herb produces flowers depending on sufficient light, temperature and soil nutrients, the that suite of cues ...
\emph{\DIFaddFL{environment's variability --}} \DIFaddFL{which aspects of the environment vary, how (e.g., temporally each year, spatially at $x$ scale) and how much }\\  \hline
...
\emph{\DIFaddFL{fundamental tracking --}} \DIFaddFL{the relationship between fitness and the difference between ideal and actual  timings; this is typically presented in an idealized way (Fig. \ref{fig:defineETorig})  but may more complicated in multivariate system (Fig. \ref{fig:defineET})}\\  \hline
\emph{\DIFaddFL{non-stationary environment --}} \DIFaddFL{the underlying distribution of a location's abiotic characteristics is unchanged across time (i.e., constant mean and variance), this suite of characteristics varies by habitat type, but generally includes physical climatic factors, such as temperature and precipitation }\\  \hline
...
\emph{\DIFaddFL{stabilizing mechanisms --}} \DIFaddFL{processes that cause species in a community to more strongly limit their fitness than other species' fitness (e.g., the common requirement for coexistence that intraspecific competition must be stronger than interspecific); includes niche differences. }\\  \hline
\emph{\DIFaddFL{stationary environment --}} \DIFaddFL{the underlying distribution of abiotic characteristics of a location changes over across time (e.g., warming temperatures, larger rainfall events)  }\\ 
\hline \hline
\end{tabular}
