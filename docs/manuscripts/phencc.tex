...
To understand the role of environmental tracking by species in variable environments we use a simple model that includes dynamics at both the intra-annual and inter-annual scales. This model thus allows within- and between-year dynamics to contribute to coexistence. Between-years the environment is included via variable germination, and within-years the environment is explicitly included as a resource pulse at the start of the season. The model includes a suite of species traits, including some relating to how the species responds to the environment via germination year and bethedges across years via a seedbank, as well as traits relating to resource competition each year. \\

As the model is akin to many commonly used seed germination models (CITES), we follow a similar terminology for ease, however the basic structure of our model could apply to others systems with one dominant pulse of a limiting resource each resource (e.g., water from rain or snowpack). %\todo[inline]{Ask Megan/check notes.}. % Conceptualization of our germination equations in other file ... Megan says we could equally assume tauI means 'I go at the same time each year and the timing of the water varies ...' ... Peter's version and this give the same answer. But we need to make clear that directionality doesn't matter -- lower tauI does not germinate before higher tauI, no sequences here! ... everyone germinates at the same time in the model!
\\

Across years, for a community of \(n\) species, the seedbank ($N$) of species $i$ at time $t$ is determined by the combination of the survival ($s_i$) of the fraction of seeds that did not germinate that season ($1-g_{i}(t)$) plus new biomass ($B_i$) produced during the length growing season ($\delta$) converted to seeds ($\phi_i$):
\begin{align}
N_{i}(t+1) & =
...
\end{align}
Each year the proportion of seeds that germinate depends on the environment, which varies through tauP, which in turn determines the . % Germination fraction varies year to year depending on the distance between tauI, a species charactersitic, tauP which varies year to year -- according to a Gaussian distribution. We conceptualize this as a variation in the timing and an environmental trigger for germination. Germination fraction declines according to a Gaussian distribution as the distance between tauI and tauP grows. tauI can be considered a fix characteristic of a species or it may respond to the environment more dynamics through tracking. Tracking decreases the distance between tauI and tauP according to this eqn .. such that a species with higher alpha will have a higher germination fraction.
\begin{align}
g_{i} & = g_{max,i}e^{-h(\tau_{p}-\tau_{i})^2} 
\end{align}
\noindent Adding phenological tracking to model: % Megan says tracking is included here! 
% tauI as a fixed timing of germination ... versus a (flexible?) timing of germination that responds to the environment ... fixed versus flexible or fixed versus tracking
\begin{align}
...
\end{align*}
% FIX below!
% My bad: Without inter-annual variability in the environment this model simplifies to R* competition. With inter-annual variability in the environment species can persist through a variety of trade-offs. 
...
% Then I say: Within-season allows for fluctuation-independent coexistence, or via variation in germination from year to year (fluctuation-depedent). 
As our interest is primarily in the role of environmental tracking we focus on situations where species vary in their match to the environment (through both tracking, $\alpha$, or a more static response, $\tau_i$) or their resource uptake (via $c_i$).
