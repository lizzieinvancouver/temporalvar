...
As both a fixed intrinsic start time and tracking represent the same major niche axis species cannot coexist given only variation in these traits; coexistence requires variation in another trait axis. As discussed above, many commonly cited traits for this trade-off revolve around traits related closely to resource competition (CITEChesson2004). With this added variation---here we varied species' $R^*$ (via c or what? I forget)---species can coexist so long as those species with a temporal niche advantage are also the inferior competition (FIGS). That is, species that can draw resources down to a lower level and are thus the superior resource competitors (lower $R^*$) can coexist with species with that are inferior competitors given those species have higher tracking values or fixed intrinsic start times close to the most common timing of the pulse (i.e., species with the superior biological start times). These trade-offs, however, are all environmentally dependent. They hold only so long as the environment is stationary. 

\subsubsection{Tracking in non-stationary environments}

% START HERE!
- Do we need to review what happens in one-species communities with no trade-offs or can we skip? (And start with sentence: A shifting environment may fundamentally reshape trade-offs that structure communities.)
- Outline is a bit faint here, may want to re-check our figures and notes in the R code and refine this (and maybe discussion) in outline...?
...
\subsubsection{Multidimensional non-stationary environments}
