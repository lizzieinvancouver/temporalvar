...
As both a fixed intrinsic start time and tracking represent the same major niche axis species cannot coexist given only variation in these traits; coexistence requires variation in another trait axis. As discussed above, many commonly cited traits for this trade-off revolve around traits related closely to resource competition (CITEChesson2004). With this added variation---here we varied species' $R^*$ (via c or what? I forget)---species can persist together so long as those species with a temporal niche advantage are also the inferior competition (FIGS). That is, species that can draw resources down to a lower level and are thus the superior resource competitors (lower $R^*$) can persist with species with that are inferior competitors given those species have higher tracking values or fixed intrinsic start times close to the most common timing of the pulse (i.e., species with the superior biological start times). These trade-offs, however, are all environmentally dependent. They hold only so long as the environment is stationary. 
...
A shifting environment may fundamentally reshape trade-offs that structure communities % START HERE! See new outline. 
