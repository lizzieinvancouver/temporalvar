...
% Should we come up with better names for tauI tauIP and tauP (esp. since we have 'tracking' for alpha)?
...
% Thinking on submitting as a R & S ...
...
We could set up more transitions between stationary and non-stationary ... and what changes (mean, variance) 
- Contrast R0, epsilon and tauP?
- Show examples .... 
-> Lake Washington 
-> Temperature records
-> Snowpack records 

- Problem with temperature sensitivity
-> threshold cues 
-> days/degree
-> plots of plants, insects, birds and then the same insects/birds as trackers of their lower level

- Do more on the angle on trade-offs with tracking ...  
-> Meta-analysis of traits that co-vary with tracking (small, quick one)
...
What does this mean for basic ecology?
- Characterizing environmental distributions better (and fitness?): Putting years of study in context. 
- 

Nonstationarity versus transient dynamics. 

Nonstationarity now versus earlier in history. 

Performance x tracking? Can we add in more data?

Links to trait literature? Not enough study of traits that include tracking components (because that's hard)


paper to check out (perhaps):
Eco-evolutionary feedbacks-Theoretical models and perspectives (Govaert)
A Critique of the Space-for-Time Substitution Practice in Community Ecolocy (Damgaard, downloaded)
Ecological change points: The strength of density dependence and the loss of history (Ponciano, TPB paper perhaps not so useful)
...
Meta-analysis (ask Mika for help) to answer:
- how much do people look at trade-offs?
- phenology can impact traits themselves, so how to analyse (competition experiments?)

How much do species track? How variable is it across (and within) species? How does it work across cues and environments?
- plants on environment tracking
- consumers on plants
- consumers on consumers
- mention hypotheses re synchrony (if linked spp do track, then how do we have differences overall across trophic levels?)

The trouble with tracking:
- multicue species may appear as single cue initially with warming
- complexity in multicue species
- biotic tracking (competition, predaction etc.)
- snowmelt date versus temp and similar correlations

How does tracking affect community coexistence?

What traits trade-off with tracking?

Conceptual figure on snowpack and temp and what they mean for modeling (use synch data for temp? Could we do a quick search of ecological studies that look at snowpack?) 
