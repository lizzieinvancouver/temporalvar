...
To understand the role of environmental tracking by species in variable environments we use a simple model akin to many commonly used seed germination models (CITES).  Our model includes dynamics at both the intra-annual and inter-annual scales, thus allowing within and between year dynamics to contribute to coexistence. The environment is included in the model between years via variable germination, and within years via a resource pulse defines the start of the season. The model includes a suite of species traits, including some relating to how the species responds to the environment via germination year and bethedges across years via a seedbank, as well as traits relating to resource competition each year. \\
