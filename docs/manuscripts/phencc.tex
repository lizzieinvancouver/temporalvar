...
As the model is akin to many commonly used seed germination models (CITES), we follow a similar terminology for ease, however the basic structure of our model could apply to others systems with one dominant pulse of a limiting resource (e.g., water from rain or snowpack). %\todo[inline]{Ask Megan/check notes.}. % Conceptualization of our germination equations in other file
\\
...
The production of new biomass each season depends on its resource uptake ($f_i(R)$ converted into biomass at rate $c_i$) less maintenance costs ($m_i$), with uptake controlled by $a_i$ and $u_i$: % ADD here!
\begin{align}
...
\end{align}
Each year the proportion of seeds that germinate depends on the environment. % ADD here
\begin{align}
g_{i} & = g_{max,i}e^{-h(\tau_{p}-\tau_{i})^2} 
\end{align}
\noindent Adding phenological tracking to model: % ADD here
\begin{align}
...
\end{align*}
Without inter-annual variability in the environment this model simplifies to R* competition and the species with the lower R* wins. With inter-annual variability in the environment species can persist through a variety of trade-offs. As our interest is primarily in the role of environmental tracking we focus on situations where species vary in their match to the environment (through both tracking, $\alpha$, or a more static response, $\tau_i$) or their resource uptake (via $c_i$).
