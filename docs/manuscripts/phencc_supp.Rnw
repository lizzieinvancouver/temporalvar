...
The competitive dynamics were modeled by simulation.  At the beginning of each run, both species were initialized with a census size of $N(0) = 100$ per unit area.  Species-specific parameters ($c_{i}$,$\tau_{i}$, and $\alpha_{i}) were drawn from uniform random distributions with the ranges given in Table S1.  All other parameters were identical between species (Table S1)
