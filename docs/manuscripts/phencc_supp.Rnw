...
@
\caption{How the environment shifts from the stationary period to the nonstationary period.  The timing of the resource pulse shifts from $\tau_{p}$ ~ $\beta(10,10)$ for the 500 year stationary period to $\tau_{p}$ ~ $\beta(5,15) over the 500 year nonstationary period.  For simulations where the amount of resource is also changing, R(0)~logNormal(log(2), 2) during the 500 year stationary period and shifts to R(0)~logNormal(log(2)/2,2) during the nonstationary period}
\label{fig:fig_Rt_tauPt}
\end{figure}

The competitive dynamics were modeled by simulation.  At the beginning of each run, both species were initialized with a census size of $N(0) = 100$ per unit area and the environment ($R(0)$ and $\tau_{p}$) was generated for the stationary and nonstationary periods..  Species-specific parameters ($c_{i}$,$\tau_{i}$, and $\alpha_{i}) were drawn from uniform random distributions with the ranges given in Table S1.  All other parameters were identical between species (Table S1).  Within-year $R{*}$ competition dynamics were solved using an ode solver ($ode$ in the R package $deSolve$), and ended when the resource was drawn down to $min(R{*})$, the $R{*}$ value of the better resources competitor.  This end of season biomass was converted to seeds for census.  At each census, a mimimum cutoff was applied to determine extinction from the model.
...
Parameter & Definition & Unit \\ \hline 
\end{tabular}
\begin{tabular}{ | p{3.0cm} | p{3.0cm} | p{6.0cm} | p{4.0cm} |}
\(N_{i}\) & \(N_{i}(0) = 100 & census of seedbank of species \(i\) & seeds per unit area \\ \hline
\(s\) & 0.8 & survival of species \(i\) & unitless \\ \hline
\(B_{i}\) & & biomass of species \(i\) & biomass \\ \hline
\(R(0)\) & ~logN(mu=log(2), sigma = 0.2) & annually varying initial value of resource at the beginning of the growing season & resource\\ \hline
\(c_{i}\) & ~unif(8,20) & conversion efficiency of \(R\) to biomass of species \(i\) &  \(\frac{\text{biomass}}{\text{resource}}\) \\ \hline
...
\(\tau_{p}\) & ~Beta(10,10) & timing of pulse & days \\ \hline
\(\tau_{i}\) & ~Unif(0.1,0.9) & timing of max germination of species \(i\) & days \\ \hline
\(\alpha_{i}\) & phenological tracking of species \(i\) & unitless \\ \hline
\hline
