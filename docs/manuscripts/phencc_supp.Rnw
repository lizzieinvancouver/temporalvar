...
\emph{Analyses}: 
The two species coexistence model (Equations 1-6) include both interannual variation in the environment and intra-annual resource competiton.  The model was modified from Chesson et al 2004, which was originally conceptualized for annual plants with a seedbank.  Although the model can be conceived of more generally, we use the language of annual plant germination for concreteness.  

Interannual variation occurs in both the timing and the amount of the resource pulse.  The timing of the resource pulse also varies from year to year and is given by \tau_{p}(t)\, which is drawn from a beta distribution. The amount of resource available at the beginning of each season, R0(t), varies from year to year and is drawn from a log-normal distribution.  Each model run is comprised of a 500 year stationary period and a 500 year nonstationary period.  During the stationary period, R0(t) and \tau_{p}(t)\ are each drawn from a stationary distribution.  During the nonstationary period, the timing of the resource pulse, \tau_{p}(t)\, gradually shifts earlier in the season (Figure S1).  For the model runs displayed in Figures 3 and 4, this is the only change in the environment during the nonstationary period.  We also discuss model runs in which both the timing, \tau_{p}(t)\, and the amount, R0(t), of the resource pulse shift during the nonstationary period.  In this case, the distribution of R0(t) also shifts gradually over the nonstationary period.    
...
Within year dynamics followed $\R{*} competition of two species for a single resource.  Differences in $\R{*}_{i} were generated by differences in conversion efficiency, $\c_{i}; species were otherwise identical in resource update parameters ($\a, $\u, and $\theta), maximum germination rate ($\g), mortality ($\m), survivorship ($\s), and conversion rates of biomass to seeds ($\phi) and seeds to biomass ($\b_{0}).  Species diffed in preferred germination time $\tau_{i} and tracking $\alpha_{i}, generating interannual differences in germination fraction.  
...
\(\delta\) (peak biomass) & total length of growing season & days\\ \hline
\(B_{i}\) & biomass of species \(i\) & biomass \\ \hline
\(R(t)\) & annually varying resource, R(t) ~ logN(lmu=log(2), sigma = 0.2) & resource\\ \hline
\(c_{i}\) & conversion of \(R\) uptake to biomass of species \(i\) &  \(\frac{\text{biomass}}{\text{resource}}\) \\ \hline
\(m_{i}\) & maintenance costs of species \(i\) & \(\text{days}^{-1}\) \\ \hline
\(a_{i}\) & uptake increase as \(R\) increases for species \(i\) & \(\text{days}^{-1}\) \\ \hline
\(u_{i}\) & max uptake for species \(i\) & \(\frac{(\text{days})(\text{biomass})}{\text{resource}}\) \\ \hline
...
\(g_{i}\) & germination fraction & unitless \\ \hline
\(\tau_{p}\) & timing of pulse, \tau_{p}~Beta(10,10) & days \\ \hline
\(\tau_{i}\) & timing of max germination of species \(i\) & days \\ \hline
\(\alpha_{i}\) & phenological tracking of species \(i\) & unitless \\ \hline
\(\theta_{i}\) & shape of uptake for species \(i\) & unitless\\ \hline
\hline
