...
phi <- rep(0.25,nsp)     # conversion of end-of-season plant biomass to seeds
...
if(nsp > 1){
  ...
  tauIPini <- mean(abs(tauP[1:nonsta[1]] - tauIhat[1:nonsta[1],]))
}
...
if (nonsta[2]>0) {
  if(nsp > 1){
    ...
    tauIPns <- mean(abs(tauP[(nonsta[1]+1):(nonsta[1]+nonsta[2])] - tauIhat[(nonsta[1]+1):(nonsta[1]+nonsta[2]),]))
  }
} else {  
  tauIPns <-  rep(NA,nsp)
}
if (nonsta[3]>0){
  if(nsp > 1){
    ...
    tauIPfin <- mean(abs(tauP[(nonsta[1]+nonsta[2]+1):nyrs] - tauIhat[(nonsta[1]+nonsta[2]+1):nyrs,]))
  }
} else {
  tauIPfin <- rep(NA,nsp)
}
