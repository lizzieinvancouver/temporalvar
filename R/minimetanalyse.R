...
#############################
dat <- read.csv("minimeta/phentrack_meta.csv", header=TRUE)
dim(dat)

length(unique(dat$paperID)) # 9 studies
...
plantshere <- subset(dat, taxongroup_studied!="Lepidoptera")
unique(plantshere$paperID) # 8 papers
butterflies <- subset(dat, taxongroup_studied=="Lepidoptera")
unique(butterflies$paperID) # 1 paper
