...
xi_tau <- c(0,runif(1,0,2))    #Dan did this to make sp1 non-dormant delay sensitivity to chill (up to 3 days per week of chill)
...
gamma_g <- (1-gmin)#/xi_100 # Dan did This and the line below to give the species the same final gp
gmax <-rep(.7,nyrs) %*% t(gamma_g) + matrix(rep(gmin,nyrs),nrow=nyrs,byrow=TRUE)
gmax <- pmin(gmax,1)  
